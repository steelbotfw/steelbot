...
static function Init($cfg) {
    ...
    $commands_list = STEELBOT_DIR.DIRECTORY_SEPARATOR.'tmp'.DIRECTORY_SEPARATOR.
        parent::$cfg['bot_uin'].'.commands.access';
...
static function SaveTimers() {
    ...
    $timers_file = STEELBOT_DIR.DIRECTORY_SEPARATOR.'tmp'.DIRECTORY_SEPARATOR.
            parent::$cfg['bot_uin'].'.timers';
    if ( file_put_contents( $timers_file, serialize(parent::$timers)) ) {
        slog::result("OK");
    } else {
        slog::result("ERROR");
    }
...
static function LoadTimers($filename = false) {
    ...
    slog::add('steelbot', 'Loading timers ... '.$filename);
 ...
 */
static function RegisterCmd($command, $func, $access = 1, $helpstr = null) {
